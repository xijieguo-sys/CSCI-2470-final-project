...
print(len(symbol_to_ncbi))

def fetch_ncbi_summary_in_batch(ncbi_ids):
    ids_str = ",".join(str(i) for i in ncbi_ids)
...
valid_gene_symbols = list(symbol_to_ncbi.keys())

for i in tqdm(range(0, len(valid_gene_symbols), BATCH_SIZE)):
    batch_symbols = valid_gene_symbols[i : i + BATCH_SIZE]
...
with open("ncbi_gene_summaries_latest.json", "w") as f:
