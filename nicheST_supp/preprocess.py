...
def get_feature(adata, param):
    ...
    if param.get('use_genept_s', False):
        feature = torch.tensor(adata.obsm['X_genept_s_gemini'], dtype=torch.float32).to(device)
        return feature
...
def prepare_graph_data(adata_ref_list, param):
    ...
    for i in range(0,len(adata_ref_list)):
        ...
        # There is no 'file_name' key in some datasets' uns dict
        sample_name = adata_ref_list[i].uns.get("file_name", f"sample_{i}")
        logger.info(f"computing {k} hop subgraph for sample {sample_name}")
...
def preprocess(args, adata, save_data=True, target_sum=1e4):
    ...
    if args.use_genept_s:
        print("Skipping HVG + Scanpy normalization because using GenePT-s embeddings.")
        return adata
